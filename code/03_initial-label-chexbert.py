...
from chexbert.bert_labeler import bert_labeler
from chexbert.bert_tokenizer import tokenize
from transformers import BertTokenizer
from collections import OrderedDict
from chexbert.unlabeled_dataset import UnlabeledDataset
...
def load_unlabeled_data(csv_path, impression_column, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS,
                        shuffle=False):
    ...
    dset = UnlabeledDataset(csv_path,impression_column)
    loader = torch.utils.data.DataLoader(dset, batch_size=batch_size, shuffle=shuffle,
                                         num_workers=num_workers, collate_fn=collate_fn)
    return loader
    
def label(checkpoint_path, csv_path,impression_column):
    ...
    """
    ld = load_unlabeled_data(csv_path,impression_column)
...
def save_preds(y_pred, csv_path, out_file,impression_column):
    """Save predictions as out_file 
    @param y_pred (List[List[int]]): list of predictions for each report
    @param csv_path (string): path to csv containing reports
    @param out_file (string): path to output directory
    """
    y_pred = np.array(y_pred)
    y_pred = y_pred.T
    
    df = pd.DataFrame(y_pred, columns=CONDITIONS)
    reports = pd.read_csv(csv_path)[impression_column]

    df[impression_column] = reports.tolist()
    new_cols = [impression_column] + CONDITIONS
    df = df[new_cols]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output_file', type=str, nargs='?', required=True,
                        help='path to intended output file')
    parser.add_argument('-c', '--checkpoint', type=str, nargs='?', required=True,
                        help='path to the pytorch checkpoint')
    parser.add_argument('-i', '--impression_column', type=str, nargs='?',
                        default="sentence",
                        help='name of column containing report text')
    args = parser.parse_args()
    csv_path = args.data
    out_file = args.output_file
    checkpoint_path = args.checkpoint
    impression_column = args.impression_column

    y_pred = label(checkpoint_path, csv_path, impression_column)
    save_preds(y_pred, csv_path, out_file,impression_column)
